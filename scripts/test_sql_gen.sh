...
SPEC_DIR=workload_spec/tpch_rq2
OUT_ROOT=workloads/tpch_rq2/SCENARIO_DEFAULT
STATS=workloads/stats/tpch_16_stats.yaml
TABLE=lineitem

mkdir -p "$OUT_ROOT/yaml" "$OUT_ROOT/sql"

for spec in "$SPEC_DIR"/spec_tpch_RQ2_*.yaml; do
...
# Amazon
# SPEC_DIR_AMZ=workload_spec/amazon_rq2
# OUT_ROOT_AMZ=workloads/amazon_rq2/SCENARIO_DEFAULT
# STATS_AMZ=workloads/stats/amazon_stats.yaml
# TABLE_AMZ=amazon

# mkdir -p "$OUT_ROOT_AMZ/yaml" "$OUT_ROOT_AMZ/sql"

# for spec in "$SPEC_DIR_AMZ"/spec_amazon_RQ2_*.yaml; do
#   name=$(basename "$spec" .yaml)
