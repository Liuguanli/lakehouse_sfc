...
for scenario_var in "${SCENARIOS[@]}"; do
  ...
  echo "[LOAD] Writing Amazon data for scenario=${name} (layouts=${layouts})"
  DELTA_LAYOUTS="$layouts" bash "${ROOT_DIR}/scripts/run_amazon_write.sh" --delta --overwrite
  ...
  echo "[CLEAN] Removing generated Amazon Delta data for scenario=${name}"
