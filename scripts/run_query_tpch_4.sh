...
# example: ./run_all.sh workloads/tpch_4_Q1
...
WORKLOAD_NAME="$(basename "$WORKLOAD_DIR")"         # last path component, e.g. tpch_4_Q1
...
ICEBERG_CONF=(
  ...
  --conf spark.sql.catalog.local.warehouse="$(pwd)/data/tpch_4/iceberg_wh"
)
...
run_delta() {
  ...
  local table_path
  case "$layout" in
    baseline) table_path="$(pwd)/data/tpch_4/delta/delta_baseline" ;;
    linear)   table_path="$(pwd)/data/tpch_4/delta/delta_linear" ;;
    zorder)   table_path="$(pwd)/data/tpch_4/delta/delta_zorder" ;;
...
run_hudi() {
  ...
  local table_path="$(pwd)/data/tpch_4/hudi/hudi_${layout}"
  "$SPARK_HOME/bin/spark-submit" \
    --packages "$HUDI_PKGS" \
    "${COMMON_CONF[@]}" \
    lakehouse_op/run_queries.py \
