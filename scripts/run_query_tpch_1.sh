...
# example: ./run_all.sh workloads/tpch_1_Q1
...
# Dataset-based directories
DATA_ROOT="./data"
ICEBERG_WH="${DATA_ROOT}/${DATASET}/iceberg_wh"
DELTA_ROOT="${DATA_ROOT}/${DATASET}/delta"
HUDI_ROOT="${DATA_ROOT}/${DATASET}/hudi"

WORKLOAD_NAME="$(basename "$WORKLOAD_DIR")"         # last path component, e.g. tpch_1_Q1
...
ICEBERG_CONF=(
  ...
  --conf spark.sql.catalog.local.warehouse="$(pwd)/data/tpch_1/iceberg_wh"
)
...
run_hudi() {
  ...
  local table_path="$(pwd)/data/tpch_1/hudi/hudi_${layout}"
  "$SPARK_HOME/bin/spark-submit" \
    --packages "$HUDI_PKGS" \
    "${COMMON_CONF[@]}" \
    lakehouse_op/run_queries.py \
